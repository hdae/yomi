...
// キャッシュヒット/ミス・self-heal（破損キャッシュの取り直し）・gzip 自動解凍・整合性検証（fail loud）を
// 検証する。依存ゼロを守るため、テスト用の最小 JTD1 バイト列は format 定数から手組みする。
...
// 既定（引数なし）で取りに行く URL の形（HF dataset・gzip・40 hex の revision で固定）。
...
Deno.test("fetchDictionaryBytes: 既定は焼き込み revision の HF gzip URL を取得する", async () => {
  ...
  try {
    ...
    assert(keys.length === 1, "保存キーが1つでない");
    assert(defaultUrlPattern.test(keys[0]), `既定 URL が想定形と不一致: ${keys[0]}`);
  } finally {
    mocks.restore();
  }
});

Deno.test("fetchDictionaryBytes: revision を上書きできる", async () => {
  const valid = buildMinimalJtd();
  const mocks = installMocks({ responseBytes: valid });
  try {
...
Deno.test("fetchDictionaryBytes: 取得物が gzip なら解凍して検証する（先頭バイト自動判定）", async () => {
  ...
  try {
    ...
    const cached = mocks.store.get("https://example.test/g-1.jtd.gz");
...
Deno.test("fetchDictionaryBytes: キャッシュヒット時は fetch しない", async () => {
  ...
  const url = "https://example.test/hit-{revision}.jtd";
  mocks.store.set("https://example.test/hit-1.jtd", valid);
  try {
    const bytes = await fetchDictionaryBytes({ url, revision: "1" });
    assert(bytes.byteLength === valid.byteLength, "返り値サイズ不一致");
...
Deno.test("fetchDictionaryBytes: 破損キャッシュは evict して network から取り直す（self-heal）", async () => {
  ...
  const mocks = installMocks({ responseBytes: valid });
  const url = "https://example.test/heal-{revision}.jtd";
  const resolved = "https://example.test/heal-1.jtd";
...
Deno.test("fetchDictionaryBytes: network の破損は throw し、壊れたものをキャッシュしない", async () => {
  ...
  let threw = false;
  try {
    await fetchDictionaryBytes({ url: "https://example.test/c-{revision}.jtd", revision: "1" });
  } catch {
    threw = true;
  } finally {
    mocks.restore();
  ...
  assert(!mocks.store.has("https://example.test/c-1.jtd"), "壊れたものをキャッシュした");
});
