...
import { loadDictionary, verifyJtd, VERSION } from "./mod.ts";
import {
  fourCC,
  HEADER_BYTES,
  MAGIC,
...
// 既定（引数なし）で loadDictionary が取りに行く URL。VERSION の焼き込みに追従する。
const defaultUrl =
...
Deno.test("loadDictionary: 既定は自身の VERSION の辞書を GitHub Release から取得する", async () => {
...
Deno.test("loadDictionary: version を上書きできる", async () => {
  const valid = buildMinimalJtd();
  const mocks = installMocks({ responseBytes: valid });
  try {
    await loadDictionary({ version: "9.9.9" });
    assert(
      mocks.store.has("https://github.com/hdae/yomi/releases/download/v9.9.9/naist-jdic-9.9.9.jtd"),
...
Deno.test("loadDictionary: url を上書きできる（{version} は version に解決）", async () => {
  const valid = buildMinimalJtd();
  const mocks = installMocks({ responseBytes: valid });
  try {
    await loadDictionary({ url: "https://example.test/d-{version}.jtd", version: "1.2.3" });
...
Deno.test("loadDictionary: キャッシュヒット時は fetch しない", async () => {
  const valid = buildMinimalJtd();
...
Deno.test("loadDictionary: 破損キャッシュは evict して network から取り直す（self-heal）", async () => {
...
Deno.test("loadDictionary: HTTP エラーは throw する（fail loud）", async () => {
  const mocks = installMocks({ responseOk: false });
  let threw = false;
  try {
    await loadDictionary({ url: "https://example.test/missing-{version}.jtd", version: "1.0.0" });
  } catch {
    threw = true;
  } finally {
    mocks.restore();
...
Deno.test("loadDictionary: network の破損は throw し、壊れたものをキャッシュしない", async () => {
  const corrupt = buildMinimalJtd(true);
  const mocks = installMocks({ responseBytes: corrupt });
  let threw = false;
  try {
    await loadDictionary({ url: "https://example.test/c-{version}.jtd", version: "1.0.0" });
  } catch {
    threw = true;
  } finally {
    mocks.restore();
