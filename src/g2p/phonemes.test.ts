...
Deno.test("nodeToMoras: 撥音・促音の擬似子音（N/cl）は子音音素でないので落とされる", () => {
  // モーラ表の内部表現は ン={consonant:"N"} / ッ={consonant:"cl"} だが（text/types.ts）、
  // g2p 層の Mora では「子音なし」= undefined に正規化する（VOICEVOX 系と同じ扱い）。
  const node = njdNode([
    njdMora("ン", "N", "N"),
    njdMora("ッ", "cl", "cl"),
    njdMora("カ", "k", "a"),
