...
// ---- 小数点スキップ状態機（本家 digit/mod.rs:37-50 と腕単位で照合済み） ----

const NAKAGURO = () => makeRuleNode("・,記号,一般,*,*,*,*,・,・,・,*/*,*");
const SAN = () => makeRuleNode("三,名詞,数,*,*,*,*,三,サン,サン,0/2,C3");

Deno.test("digit(小数点): 数・数 の中黒は小数点「．」（テン・結合）へ置換される", () => {
  const out = njdSetDigit([ICHI(), NAKAGURO(), NI()]);
  assertEquals(surfaces(out), ["一", "．", "二"], "・→テン置換");
  assertEquals(kanaOf(out[1]), "テン", "読みはテン");
  assertEquals(out[1].chainFlag, true, "テンは前と結合");
});

Deno.test("digit(小数点): 連続セパレータ「一・二・三」は2つ目が判定に入らず記号のまま残る", () => {
  // 置換直後に skip=ifMeishi→skipping と遷移するため、2つ目の・は prev/next が数でも
  // 判定に入らず素通りする（本家のスキップ状態機と同挙動）。
  const out = njdSetDigit([ICHI(), NAKAGURO(), NI(), NAKAGURO(), SAN()]);
  assertEquals(surfaces(out), ["一", "．", "二", "・", "三"], "2つ目は変換されない");
  assertEquals(out[3].pos[0], "記号", "2つ目は記号ノードのまま");
});

Deno.test("digit(小数点): 「一・二・三・四」は1個おきにテン化（3つ目で状態機が再武装）", () => {
  // 2つ目の・（記号=非名詞）で skipping→disabled に戻るため、3つ目は再びテン化される。
  const out = njdSetDigit([ICHI(), NAKAGURO(), NI(), NAKAGURO(), SAN(), NAKAGURO(), YON()]);
  assertEquals(surfaces(out), ["一", "．", "二", "・", "三", "．", "四"], "1個おきにテン化");
  assertEquals([kanaOf(out[1]), kanaOf(out[5])], ["テン", "テン"], "1・3つ目の読み");
});
