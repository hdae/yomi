...
const installMocks = (opts: {
  ...
  revisionSha?: string;
  /** 指定時、辞書本体の応答をこの Promise の解決まで遅延する（並行合流のテスト用）。 */
  holdDict?: Promise<void>;
}) => {
  ...
  globalThis.fetch = ((input: string) => {
    ...
    return opts.holdDict === undefined ? Promise.resolve(respond()) : opts.holdDict.then(respond);
  }) as typeof fetch;
...
Deno.test("fetchDictionaryBytes: 並行呼び出しは 1 フライトに合流し重複 DL しない", async () => {
  // fetch-cache 0.3.0 の single-flight（同一 cacheName+URL の合流）に委譲した契約を
  // ローダの外形として凍結する（旧 known-issues W-E-7 のクローズ）。
  const valid = buildMinimalJtd();
  const gate = Promise.withResolvers<void>();
  const mocks = installMocks({ responseBytes: valid, holdDict: gate.promise });
  const revision = sha("9");
  try {
    const first = fetchDictionaryBytes({
      url: "https://example.test/sf-{revision}.jtd",
      revision,
    });
    const second = fetchDictionaryBytes({
      url: "https://example.test/sf-{revision}.jtd",
      revision,
    });
    // 両呼び出しが合流点へ確実に到達してから応答を解放する（macrotask 境界で microtask を掃く）。
    await new Promise((resolve) => setTimeout(resolve, 0));
    gate.resolve();
    const [a, b] = await Promise.all([first, second]);
    assert(a.byteLength === valid.byteLength, "1本目のサイズ不一致");
    assert(b.byteLength === valid.byteLength, "2本目のサイズ不一致");
    assert(
      mocks.fetchCalls() === 1,
      `重複 DL を抑止できていない: fetch ${mocks.fetchCalls()} 回`,
    );
  } finally {
    mocks.restore();
  }
